...
# ── V4: /api/worldbuild  世界观构建 ────────────────────────────────────────────

class WorldbuildRequest(BaseModel):
    premise: str          # 一句话设定
    genre: str = "玄幻"   # 题材

@app.post("/api/books/{book_id}/worldbuild")
def api_worldbuild(book_id: str, req: WorldbuildRequest):
    """世界观构建：从一句话设定生成完整世界观（V4 新增）"""
    from dotenv import load_dotenv
    load_dotenv(ENV_PATH, override=True)
    try:
        cmd = [
            sys.executable, "-m", "cli.main", "worldbuild",
            req.premise, "--genre", req.genre,
            "--book", book_id,
        ...
        return {"ok": result.returncode == 0, "stdout": result.stdout[-3000:], "stderr": result.stderr[-1000:]}
    except subprocess.TimeoutExpired:
        return {"ok": False, "stdout": "", "stderr": "世界观构建超时（>5分钟）"}
...
class OutlineRequest(BaseModel):
    book_id: str
    chapters: int = 90          # 总章节数
    target_words: int = 2000    # 每章字数

@app.post("/api/books/{book_id}/outline")
def api_outline(book_id: str, req: OutlineRequest):
    """大纲规划：从世界观生成三幕结构+章纲（V4 新增）"""
    from dotenv import load_dotenv
    load_dotenv(ENV_PATH, override=True)
    try:
        cmd = [
            sys.executable, "-m", "cli.main", "outline",
            "--book", book_id,
            "--chapters", str(req.chapters),
            "--target-words", str(req.target_words),
            "--project", str(PROJECT_ROOT),
        ]
        result = subprocess.run(
        ...
        return {"ok": False, "stdout": "", "stderr": "大纲规划超时（>10分钟）"}
...
class MarketRequest(BaseModel):
    genre: str                    # 题材
    premise: str = ""             # 一句话设定
    platform: str = "番茄小说"     # 目标平台

@app.post("/api/action/market")
def api_market(req: MarketRequest):
    """市场分析：分析目标读者偏好（V4 增强：注入番茄市场数据）"""
    from dotenv import load_dotenv
    load_dotenv(ENV_PATH, override=True)
    try:
        cmd = [sys.executable, "-m", "cli.main", "market", req.genre]
        if req.premise:
            cmd.extend(["--premise", req.premise])
        if req.platform:
        ...
        return {"ok": False, "stdout": "", "stderr": "市场分析超时"}


# ── V4: /api/quality-dashboard  质量仪表盘数据 ─────────────────────────────────

@app.get("/api/books/{book_id}/quality-dashboard")
def api_quality_dashboard(book_id: str):
    """获取质量仪表盘数据（V4 新增 Web UI 端点）"""
    sm = _sm(book_id)
    dash_path = sm.book_dir / "quality_dashboard.json"
...
# ── V4: /api/kb-queries  知识库查询统计 ────────────────────────────────────────

@app.get("/api/books/{book_id}/kb-queries")
def api_kb_queries(book_id: str):
    """获取知识库查询统计数据（V4 新增）"""
    sm = _sm(book_id)
    kb_path = sm.book_dir / "kb_queries.json"
    if not kb_path.exists():
        return {"records": [], "summary": {"total": 0, "by_role": {}, "by_file": {}}}
    try:
        ...
        for rec in records:
            for q in rec.get("queries", []):
                ...
                by_file[q["file"]] = by_file.get(q["file"], 0) + 1
        return {
            "records": records,
            "summary": {"total": total, "by_role": by_role, "by_file": by_file},
        }
    except Exception:
        return {"records": [], "summary": {"total": 0, "by_role": {}, "by_file": {}}}
